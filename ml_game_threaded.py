...
MODEL = os.path.expanduser("~/models/hand.tflite")  # drop any small .tflite here
...
# Threaded camera capture globals
...
def make_interpreter(use_delegate):
    ...
    # Toggle XNNPACK too, so ON/OFF shows a difference even without ArmNN
    if use_delegate:
...
def camera_thread():
    """Dedicated camera capture thread - runs independently, never blocks game loop"""
    global latest_camera_frame, camera_running

    # FIX: Disable dynamic framerate to unlock 30 FPS (Logitech Brio 101 issue)
    try:
    ...
    # Fast camera: V4L2 backend + MJPG @ 640x360 @ 30fps
...
def loop():
    ...
    while True:
        ...
        with camera_frame_lock:
            fr = latest_camera_frame.copy() if latest_camera_frame is not None else None
        ...
        if frames_waited > 0:
            print(f"Camera ready after {frames_waited} waits")
            frames_waited = 0
